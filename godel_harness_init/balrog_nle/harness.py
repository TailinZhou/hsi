...
task_context keys (set by evaluator each step):
    env_name          str   — game environment name (e.g. "NetHackChallenge")
...
def _inject_short_term(messages: List[Dict], short_term: str) -> List[Dict]:
    ...
    the original BALROG where only the latest observation carries inventory.
...
class HarnessPolicy:
    ...
    def _build_context(self, state: Dict, task_ctx: Dict) -> str:
        ...
        - short_term (inventory) is NOT baked into history messages;
          it is injected into the latest user message at LLM call time only.
        - naive_instruction is appended to the last user message by
          hook_on_request (hooks.py) — do NOT repeat it here.
        """
        parts = []
    ...
    def workflow(self, agent, obs, ctx, hooks_mod, prompts_mod) -> str:
        ...
        Key design:
        - short_term is NOT baked into the user message added to history.
          It is injected into the latest message only at LLM call time,
          so historical observations contain only long_term context.
        - naive_instruction is appended to the last user message by
          hook_on_request (hooks.py), matching original BALROG behavior.
