...
task_context keys (set by evaluator each step):
    env_name          str   — game environment name (e.g. "textworld")
...
class HarnessPolicy:
    ...
    def _build_context(self, state: Dict, task_ctx: Dict) -> str:
        ...
        Note: instruction (action list + goal) is appended to the system prompt
    ...
    def workflow(self, agent, obs, ctx, hooks_mod, prompts_mod) -> str:
        ...
        action, raw_msg = self.get_agent_decision(agent, ctx.get_messages(), hooks_mod)
    ...
    def get_agent_decision(self, agent, messages, hooks_mod):
        """Single agent.call_llm() call. Returns (action_text, raw_message).

        Uses call_llm() (not react()) because the harness doesn't need the full
        evolution-loop pipeline — it just needs a short text response.  The
        harness.max_tokens config caps output to prevent runaway token generation.
        """
        response = agent.call_llm(messages=messages, tools=None)
