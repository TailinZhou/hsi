...
from benchmark.balrog.environments.env_wrapper import EnvWrapper
...
def make_env(env_name, task, config, render_mode=None):
    ...
    """
    if env_name == "nle":
        from benchmark.balrog.environments.nle.nle_env import make_nle_env

        base_env = make_nle_env(env_name, task, config, render_mode=render_mode)
    elif env_name == "minihack":
        from benchmark.balrog.environments.minihack.minihack_env import make_minihack_env

        base_env = make_minihack_env(env_name, task, config, render_mode=render_mode)
    elif env_name == "babyai":
        from benchmark.balrog.environments.babyai_text.babyai_env import make_babyai_env

        base_env = make_babyai_env(env_name, task, config, render_mode=render_mode)
    elif env_name == "crafter":
        from benchmark.balrog.environments.crafter.crafter_env import make_crafter_env

        base_env = make_crafter_env(env_name, task, config, render_mode=render_mode)
    elif env_name == "textworld":
        from benchmark.balrog.environments.textworld.textworld_env import make_textworld_env

        base_env = make_textworld_env(env_name, task, config, render_mode=render_mode)
    elif env_name.startswith("babaisai"):
        from benchmark.balrog.environments.babaisai.babaisai_env import make_babaisai_env
