...
def describe_loc_precise(ref, P):
    ...
    def distance_to_string(distance, direction):
        return f"{abs(distance)} step{'s' if abs(distance) > 1 else ''} {direction}"

    if ref[1] > P[1]:
        desc.append(distance_to_string(ref[1] - P[1], "north"))
    elif ref[1] < P[1]:
        desc.append(distance_to_string(ref[1] - P[1], "south"))
    if ref[0] > P[0]:
        desc.append(distance_to_string(ref[0] - P[0], "west"))
    elif ref[0] < P[0]:
        desc.append(distance_to_string(ref[0] - P[0], "east"))
...
def describe_env(
    ...
    result = ""
    _describe_loc = describe_loc_precise if precise_location else describe_loc_old
    obj_info_list = []
    ...
    for i in range(semantic.shape[0]):
        for j in range(semantic.shape[1]):
            ...
            obj_info_list.append((id_to_item[idx], _describe_loc(np.array([0, 0]), np.array([i, j]) - center)))
