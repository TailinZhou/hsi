...
class EnvWrapper(gym.Wrapper):
    ...
    def _process_observation(self, obs):
        if self.env_name in ["nle", "minihack"]:
            ...
            obs = obs
        elif self.env_name.startswith("babaisai"):
            obs = obs
    ...
    def get_instruction_prompt(self, instructions=None):
        if self.env_name == "nle":
            from benchmark.balrog.environments.nle import get_instruction_prompt

            return get_instruction_prompt()
        elif self.env_name == "minihack":
            from benchmark.balrog.environments.minihack import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "babyai":
            from benchmark.balrog.environments.babyai_text import get_instruction_prompt

            return get_instruction_prompt(self.env, mission=instructions)
        elif self.env_name == "textworld":
            from benchmark.balrog.environments.textworld import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name.startswith("babaisai"):
            from benchmark.balrog.environments.babaisai import get_instruction_prompt

            return get_instruction_prompt(self.env, self.task_name)
        elif self.env_name == "crafter":
            from benchmark.balrog.environments.crafter import get_instruction_prompt
    ...
    def get_action_list_str(self) -> str:
        space = getattr(self.env, 'language_action_space', None)
        if space is None:
            return ""
        if type(space).__name__ == 'AlwaysTrue':
            return "any text command (e.g., look, go north, take coin)"
        try:
