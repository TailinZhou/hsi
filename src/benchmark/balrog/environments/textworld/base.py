...
workspace_dir = importlib.resources.files("benchmark.balrog").__str__()

# tatsu's _PARSER is a module-level singleton in textworld.logic and NOT thread-safe:
#   parse() → _reset() → _initialize_caches() clears _rule_stack.
# Every call that reaches _PARSER.parse() (gym.make, env.reset) must hold this lock.
_textworld_parser_lock = threading.Lock()
    ...
    def get_textworld_env(self, task, seed=None, **kwargs):
        ...
        if seed is not None:
            ...
            with self._count_lock:
                self.count[task] += 1
                idx = self.count[task]
            env_id = self.env_ids[task][idx % len(self.env_ids[task])]

        with _textworld_parser_lock:
